...
def run_simulation( model, store_data, random_seed, max_time = 3, min_com_height = 0.3 ):
	...
	for t in np.arange( 0, max_time, 0.01 ):
		...
		# The internal step size is variable, and determined by the 'accuracy' setting in the .scone file
		...
		if com_y < min_com_height:
			print( f'Aborting simulation at t={model.time():.2f} com_y={com_y:.8f}' )
			break

	# Write results to the SCONE results folder
	# The resulting .sto file can be openend directly in SCONE Studio for analysis
	if store_data:
		model.write_results( 'sconepy_example' )
		print( 'Results written to "sconepy_example"; please use SCONE Studio to replay the .sto file.', flush=True )
...
# model = sconepy.load_model( 'H0918_hyfydy.scone' )
...
measure_performance( 'H0918_hyfydy.scone' )
