...
PYBIND11_MODULE( sconepy, m ) {
	...
	py::class_<scone::Model>( m, "Model" )
		...
		.def( "set_custom_value", &scone::Model::SetUserValue, "Set a custom value that can be accessed by other components" )
		.def( "get_custom_value", &scone::Model::GetUserValue, "Get a custom value previously set by set_custom_value or another component" )
		.def( "has_custom_value", &scone::Model::HasUserValue, "Check if a custom value exists" )
