...
#include "xo/numerical/piecewise_cubic_spline.h"

namespace scone
{
	/// Parameterizable piece-wise cubic spline.
	class SCONE_API PieceWiseCubicSpline : public Function
	{
	public:
		PieceWiseCubicSpline( const PropNode& props, Params& par );
		virtual ~PieceWiseCubicSpline() = default;
		...
		/// Parameter for the slope or first order derivative of each control point.
		const PropNode& control_point_dy;
	...
	private:
		xo::piecewise_cubic_spline<Real> m_Func;
	};
}
